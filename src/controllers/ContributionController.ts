...
export class ContributionController {
    ...
    public add(req: Request, res: Response) {
        ...
        const addContribution = () => {
            this.db.collection(Constants.DB_COLLECTIONS.CONTRIBUTION).insertOne(contribution)
            .then(_dbResult => {
                if (_dbResult) {
                    return res.json({
                        code: 0,
    ...
    public get(req: Request, res: Response) {
        ...
        const findContributions = (_userId: string) => {
            const cursor = this.db.collection(Constants.DB_COLLECTIONS.CONTRIBUTION).find({userId: _userId});
            const contributions = [];
            cursor.forEach(
